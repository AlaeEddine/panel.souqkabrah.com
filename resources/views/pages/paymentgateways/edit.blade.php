...
<div class="d-sm-flex align-items-center justify-content-between mb-4 rtl">
    <h1 class="h3 mb-0 text-gray-800">{{  e($PaymentGateway->name) }}</h1>
</div>
<div class="row rtl">
    <div class="col-12">
        <div class="card shadow mb-4">
            <div class="card-header py-3">
                <h6 class="m-0 font-weight-bold text-primary">{{ e($PaymentGateway->name)}}</h6>
            </div>
            <div class="card-body">
                <div class="container">
                    <div class="card-body pt-0">
                        ...
                        @endif
                        @can('update-payment-gateways')
                        <form action="{{ route('paymentgateways.edit.submit', e($PaymentGateway->id)) }}" method="POST">
                            @csrf
                            <div class="row">
                                <div class="form-group col-md-6 col-12">
                                    <label for="name" class="text-info">{{__('إسم الشركة')}}</label>
                                    <input type="text" class="form-control" name="name" id="name" placeholder="{{__('إسم الشركة')}}" value="{{e($PaymentGateway->name)}}">
                                </div>
                                <div class="form-group col-md-6 col-12">
                                    <label for="description" class="text-info">{{__('وصف الشركة')}}</label>
                                    <input type="text" class="form-control" name="description" id="description" placeholder="{{__('وصف الشركة')}}" value="{{$PaymentGateway->description}}">
                                </div>
                                <h2>الحساب</h2>
                                <div class="row">
                                    <div class="form-group col-md-6 col-12">
                                    ...
                                    <div class="form-group col-md-6 col-12">
                                        <label for="valide" class="text-info">{{__('تفعيل بوابة الدفع')}}</label>
                                        <select name="valide" id="valide" class="custom-select">
                                            <option value="0" @if($PaymentGateway->valide == 0) selected @endif>{{ __('لا') }}</option>
                                            <option value="1" @if($PaymentGateway->valide == 1) selected @endif>{{ __('نعم') }}</option>
                                        </select>
                                    </div>
                                </div>
                            ...
                            <div class="form-group col-md-6 col-12">
                                <input type="submit" formaction="{{ route('paymentgateways.edit.submit', e($PaymentGateway->id)) }}" class="btn btn-primary mb-2" value="{{__('حفظ البيانات')}}">
                            </div>
                        </form>
                        <p><br></p>
                        @endcan
